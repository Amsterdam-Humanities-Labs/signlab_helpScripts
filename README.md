# signlab_helpScripts
One-off and scheduled helper scripts from the core server (/web/helpScripts).

## What it does
Code copied read-only from `/web/helpScripts` on the core server (2026-09-22), so it is no longer only on one machine. Data files, logs and caches were left out.

## Where it runs
The core server, `/web/helpScripts`. The server still runs its own copy; this repo is not deployed yet.
