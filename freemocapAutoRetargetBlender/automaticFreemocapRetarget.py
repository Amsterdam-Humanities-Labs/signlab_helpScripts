...
def main():
    ...
    blender_path = "/home/signcollect/blender-4.0.2-linux-x64/blender"
    ...
    # scripts.extend([
    #             os.path.join(script_dir, "loadPlugins.py")
    # ])
    ...
    for root, dirs, files in os.walk(args.folder):
        for file in files:
            if file.endswith('.blend'):
                # Construct the path to the corresponding .glb file in the ../out directory
                blend_file_path = os.path.join(root, file)
                parent_dir = os.path.dirname(blend_file_path)
                out_dir = os.path.join(parent_dir, '..', 'out')
                glb_file_path = os.path.join(out_dir, file.replace('.blend', '.glb'))
